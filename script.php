...
<div class="time">
    ...
    if (!is_numeric($Y)) {
        ...
        if (($X >= 0 && $X <= $R / 2 && $Y <= 0 && $Y >= -$R)) {
            $IN = "IN";
        } elseif (IsInCircle($R, $X, $Y)) {
            $IN = "IN";
        } elseif (IsInTriangle($A_X, $A_Y, $B_X, $B_Y, $C_X, $C_Y, $X, $Y)) {
            $IN = "IN";
        } else {
            $IN = "OUT";
        }
...
<div class="table">
      <span style="font-size: x-small; font-family: Courier New,monospace; ">
...
<div class="submit">
    <button onclick="history.go(-1);" class="btn-submit sharp">Back</button>
</div>
</body>
</html>
